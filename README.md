...
| 6 | **Suggestions** | Paste a request for information that just arrived. Precedent is retrieved from the same application section, and up to three options are proposed — *supply the document*, *justify what was filed*, *commit to a date* — each cited to the records it was built from, each with the reason not to pick it. Below the similarity threshold it refuses and escalates. Citations that name records outside the retrieved set are dropped, and an option left with none is discarded. |
