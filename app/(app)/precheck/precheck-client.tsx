...
function Results({ result, ranFor }: { result: PrecheckResult; ranFor?: PrecheckState['ranFor'] }) {
    ...
    <section className="mt-8">
      ...
      <div className="rounded-lg border border-border bg-surface p-4">
        ...
        <p className="mt-1 text-[13px] text-muted">
          ...
          {result.rules.length} mined rule{result.rules.length === 1 ? '' : 's'} evaluated,{' '}
...
export function PrecheckClient({
  ...
  return (
    <>
      ...
      {state.result && <Results result={state.result} ranFor={state.ranFor} />}
    </>
  )
}
